...
if (!session_id() && (!defined('NO_SESSION') || !NO_SESSION)) {
    $domain = sprintf('.%s', $_SERVER['HTTP_HOST']);
    $currentCookieParams = session_get_cookie_params();

    $isSecure = (isset($_SERVER['SERVER_PORT']) && $_SERVER['SERVER_PORT'] == 443) ||
                (isset($_SERVER['HTTPS']) && in_array(strtolower($_SERVER['HTTPS']), ['on', '1']));
