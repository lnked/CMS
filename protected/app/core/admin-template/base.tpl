...
<head>
    ...
    {if !empty($reactFiles.styles)}
        {foreach $reactFiles.styles as $file}
        <link rel="stylesheet" href="/cp/react/{$file}" />
        {/foreach}
    {/if}
    ...
    {literal}
    <script>
        Raven.config('https://b1633193741740e58c7122d526174117@sentry.io/1238565').install()
    </script>
    {/literal}
...
<div class="page" id="page">
	...
	<section class="wrapper clearfix" id="page-wrapper">
		...
		{* <article class="content clearfix"{if $isReact} id="app"{/if}> *}
...
{*
{if !empty($reactFiles.scripts)}
    {foreach $reactFiles.scripts as $file}
    <link rel="stylesheet" href="/cp/react/{$file}" />
    {/foreach}
{/if}
*}
...
{if $isReact}
<script src="/{$ADMIN_DIR}/react/bundle.min.js" defer></script>
<script src="/{$ADMIN_DIR}/react/startup.js" defer></script>
<script src="/{$ADMIN_DIR}/react/client.min.js" async></script>
{/if}
